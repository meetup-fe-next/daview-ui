import { GITHUB_REPOSITORY_API_URL, REVALIDATE_GITHUB_DATA } from '@/constants';
...
const getCategories = async () => {
  const res = await fetch(`${GITHUB_REPOSITORY_API_URL}`, {
    next: { revalidate: REVALIDATE_GITHUB_DATA },
  });
  const resJson: GithubContentEntry[] = await res.json();
  const data = resJson.filter(({ name }) => name !== 'README.md').filter(({ name }) => name !== '.github');
...
const getCreators = async () => {
  ...
  for (const { path } of categories) {
    const res = await fetch(`${GITHUB_REPOSITORY_API_URL}/${path}`, {
      next: { revalidate: REVALIDATE_GITHUB_DATA },
    });
...
const githubLegacySdk = {
  getCategories,
  getCreators,
  getLectures,
};

export default githubLegacySdk;
